...
def ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon):
    ...
    grbModel.addConstrs(quicksum(Y_ijm[s,m,i,j] for i in range(Manufacturing_plants))
                          >= quicksum(Z_jkm[s,m,j,k] for k in range(Market))
...
def PrintToFileSummaryResults(num_Scenarios):
    results_file = "/home/dkabe/Model_brainstorming/SAA_Analysis_v6/Instance_2/Objectives/" + str(num_Scenarios) + "_results" + ".txt"
...
def SaveOpeningDecisions(num_Scenarios, batch):
    results_file = "/home/dkabe/Model_brainstorming/SAA_Analysis_v6/Instance_2/Opening_Decisions/" + str(num_Scenarios) + "_scenarios/" + str(num_Scenarios) + "_" + str(batch) + "_opening_decisions" + ".txt"
