...
demand = np.loadtxt("/home/dkabe/Model_brainstorming/SAA_Analysis_v2/demand.txt")
...
def SetGrb_Obj(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced):
    ...
    for s in range(num_Scenarios):
        for k in range(Market):
            for m in range(Products):
                rl_penalty += Probabilities[s]*lost_sales[instance][k][m]*w_s[s,k,m]*demand[m][k]
...
def ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon):
    ...
    grbModel.addConstrs(quicksum(Z_jkm[s,m,j,k] for j in range(Distribution)) +
                        ...
                        U_km[s,k,m] >= demand[m][k] for s in range(num_Scenarios) for m in range(Products)
                        for k in range(Market))
    ...
    grbModel.addConstrs(w_s[s,k,m] >= rl - (1 - U_km[s,k,m]/demand[m][k]) for s in range(num_Scenarios) for k in range(Market) for m in range(Products))
...
def get_rl_rate(w, instance, num_Scenarios, Market, Products):
    ...
    for s in range(num_Scenarios):
        for k in range(Market):
            for m in range(Products):
                rl_penalty += Probabilities[s]*lost_sales[instance][k][m]*w[s,k,m]*demand[m][k]

    return(rl_penalty)

def PrintToFileSummaryResults(num_Scenarios):
    results_file = "/home/dkabe/Model_brainstorming/SAA_Analysis_v2/Objectives/" + str(num_Scenarios) + "_results" + ".txt"
...
def SaveOpeningDecisions(num_Scenarios, batch):
    results_file = "/home/dkabe/Model_brainstorming/SAA_Analysis_v2/Opening_Decisions/" + str(num_Scenarios) + "_scenarios/" + str(num_Scenarios) + "_" + str(batch) + "_opening_decisions" + ".txt"
...
def run_Model(num_Scenarios, batch, instance=1, rl=0.5, Manufacturing_plants=6, Distribution=4, Market=29, Products=3, Outsourced=3, epsilon=700000):
