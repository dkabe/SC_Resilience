...
text_file = open("/home/dkabe/Model_brainstorming/SAA_Analysis_v2/Evaluation_Set.txt", "r")
...
def InitializeModelParams(num_Scenarios, batch):
    global x_i
    global x_j
    path = "/home/dkabe/Model_brainstorming/SAA_Analysis_v2/Opening_Decisions/"
...
def SetGrb_Obj(instance, Manufacturing_plants, Distribution, Market, Products, Outsourced, s1):
    ...
    for k in range(Market):
        for m in range(Products):
            rl_penalty += lost_sales[instance][k][m]*w_s[k,m]*demand[m][k]
...
def ModelCons(instance, rl, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, s1):
    ...
    grbModel.addConstrs((quicksum(Z_jkm[m,j,k] for j in range(Distribution)) +
                         quicksum(T_lkm[m,l,k] for l in range(Outsourced)) + U_km[k,m]) >= demand[m][k]
    ...
    # Resilience Metric 
    grbModel.addConstrs(w_s[k,m] >= rl - (1 - U_km[k,m]/demand[m][k]) for k in range(Market) for m in range(Products))
...
def get_rl_rate(w, instance, num_Scenarios, Market, Products):
    ...
    for s in range(num_Scenarios):
        for k in range(Market):
            for m in range(Products):
                rl_penalty += lost_sales[instance][k][m]*w[s,k,m]*demand[m][k]

    return(rl_penalty)

def PrintToFileSummaryResults(num_Scenarios, batch):
    results_file = "/home/dkabe/Model_brainstorming/SAA_Analysis_v2/Upper_Bounds/" + str(num_Scenarios) + "_scenarios/" + str(num_Scenarios) + "_" + str(batch) + "_UB_results" + ".txt"
