...
def SetGurobiModel(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, e_constraint):
    ...
    ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, e_constraint)
...
def SolveModel(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced):
    ...
    #obj_1 = (Cost_dict["f1"] + Cost_dict["f2"])    
    #obj_2 = (Cost_dict["f3"] + Cost_dict["f4"])
    return
...
def ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, e_constraint):
    ...
    if f1:
        grbModel.addConstr(quicksum(f_i[instance][i]*x_i[i] for i in range(Manufacturing_plants)) + quicksum(f_j[instance][j]*x_j[j] for j in range(Distribution)) +
                        ...
                        + quicksum(Probabilities[instance][s]*T_O_MZ[instance][m][l][k]*T_lkm[s,m,l,k] for s in range(num_Scenarios) for m in range(Products) for l in range(Outsourced) for k in range(Market)) <= ranges[0][e_constraint])
    ...
    if f2:
        grbModel.addConstr(quicksum(Probabilities[instance][s]*lost_sales[instance][k][m]*U_km[s,k,m] for s in range(num_Scenarios) for m in range(Products) for k in range(Market)) 
                         + quicksum(Probabilities[instance][s]*lost_sales[instance][k][m]*w_s[s,k,m]*demand[instance][s][m][k] for s in range(num_Scenarios) for m in range(Products) for k in range(Market)) <= ranges[1][e_constraint])
...
def run_Model(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, objDict, f1, f2, e_constraint):
    for key, value in objDict.items():
        objWeights[key] = value

    InitializeModelParams(instance, rl)
    SetGurobiModel(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, e_constraint)
