...
def InitializeModelParams(instance, rl):
    global ranges
    temp_path = '/home/dkabe/Model_brainstorming/Epsilon_Constraint/Three_Objectives/Instance_' + str(instance + 1) + '/Ranges/' + str(rl) + '_' + 'ranges.txt'
    ranges = np.loadtxt(temp_path)
   
def SetGurobiModel(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, f3, e1, e2, e3):
    ...
    ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, f3, e1, e2, e3)
...
def ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, f3, e1, e2, e3):
    ...
    if f1:
        grbModel.addConstr(quicksum(f_i[instance][i]*x_i[i] for i in range(Manufacturing_plants)) + quicksum(f_j[instance][j]*x_j[j] for j in range(Distribution)) +
                            ...
                            + quicksum(Probabilities[instance][s]*Manufacturing_costs[instance][i][m]*Q_im[s,m,i] for s in range(num_Scenarios) for m in range(Products) for i in range(Manufacturing_plants))<= ranges[0][e1])
    ...
    if f2:
        grbModel.addConstr(quicksum(Probabilities[instance][s]*(Supplier_cost[instance][0][m][l]*V1_lm[s,m,l] + Supplier_cost[instance][1][m][l]*V2_lm[s,m,l]) for s in range(num_Scenarios) for m in range(Products) for l in range(Outsourced))
                        + quicksum(Probabilities[instance][s]*T_O_MZ[instance][m][l][k]*T_lkm[s,m,l,k] for s in range(num_Scenarios) for m in range(Products) for l in range(Outsourced) for k in range(Market)) <= ranges[1][e2])
    ...
    if f3:
        grbModel.addConstr(quicksum(Probabilities[instance][s]*lost_sales[instance][k][m]*U_km[s,k,m] for s in range(num_Scenarios) for m in range(Products) for k in range(Market)) 
                         + quicksum(Probabilities[instance][s]*lost_sales[instance][k][m]*w_s[s,k,m]*demand[instance][s][m][k] for s in range(num_Scenarios) for m in range(Products) for k in range(Market)) <= ranges[2][e3])
...
def PrintToFileSummaryResults(instance, rl):
    results_file = '/home/dkabe/Model_brainstorming/Epsilon_Constraint/Three_Objectives/Instance_' + str(instance + 1) + '/e_cons_results_' + str(rl) + '.txt'
    if grbModel.status == GRB.OPTIMAL:
        ff = open(results_file, "a")
        ff.write(str(Cost_dict["f1"]) + '\t' + str(Cost_dict["f2"]) + '\t' + str(Cost_dict["f3"] + Cost_dict["f4"]))
        ff.write('\n')
        ff.close()
    return

def run_Model(e1, e3, instance = 0, rl = 0.5, num_Scenarios = 192, Manufacturing_plants = 6, Distribution = 4, Market = 29, Products = 3, Outsourced = 3, epsilon = 1500000, objDict = {'f1': 0, 'f2': 1, 'f3': 0}, f1 = 1, f2 = 0, f3 = 1, e2 = 1):
    for key, value in objDict.items():
        objWeights[key] = value

    InitializeModelParams(instance, rl)
    SetGurobiModel(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon, f1, f2, f3, e1, e2, e3)
