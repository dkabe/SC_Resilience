...
def InitializeModelParams(num_Scenarios, Market, Products, batch):
    global Scenarios
    global demand
    path = '/home/dkabe/SC_Resilience/SAA_Analysis_v7/Instance_1/'
...
def SetGrb_Obj(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced):
    ...
    for s in range(num_Scenarios):
        for k in range(Market):
            for m in range(Products):
                rl_penalty += lost_sales[instance][k][m]*w_s[s,k,m]*demand[s][m][k]
...
def ModelCons(instance, rl, num_Scenarios, Manufacturing_plants, Distribution, Market, Products, Outsourced, epsilon):
    ...
    grbModel.addConstrs(quicksum(volume[instance][m]*Y_ijm[s,m,i,j] for i in range(Manufacturing_plants) for m in range(Products)) <=
                        Scenarios[s][1][j]*Capacities_j[instance][j]*x_j[j] for s in range(num_Scenarios)
                        for j in range(Distribution))
...
def PrintToFileSummaryResults(num_Scenarios):
    results_file = "/home/dkabe/SC_Resilience/SAA_Analysis_v7/Instance_1/Objectives/" + str(num_Scenarios) + "_results" + ".txt"
...
def SaveOpeningDecisions(num_Scenarios, batch):
    results_file = "/home/dkabe/SC_Resilience/SAA_Analysis_v7/Instance_1/Opening_Decisions/" + str(num_Scenarios) + "_scenarios/" + str(num_Scenarios) + "_" + str(batch) + "_opening_decisions" + ".txt"
...
def run_Model(num_Scenarios, batch, instance=0, rl=0.5, Manufacturing_plants=6, Distribution=4, Market=29, Products=3, Outsourced=3, epsilon=1500000):
